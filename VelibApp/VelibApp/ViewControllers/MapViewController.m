...
- (void)setStationAnnotationOnMap {
    ...
    for (NSInteger i=0; i<[self.stationArray count]; i++) {
        Station *station = [self.stationArray objectAtIndex:i];
        CLLocationCoordinate2D locationCoord = CLLocationCoordinate2DMake([station.lat doubleValue], [station.lng doubleValue]);
        
        StationAnnotation *annotation = [[StationAnnotation alloc] initWithCoordinate:locationCoord title:station.name andSubtitle:[NSString stringWithFormat:@"%ld bikes available",[station.nbBikeAvailable integerValue]]];
